...
//save data info to data_IHDR struct
    ...
    printf( "(%d, %d, %d)\n", ntohl(png_IHDR_data->width), ntohl(png_IHDR_data->height), png_IHDR_data->bit_depth );
...
//save IDAT datafield to tmp buffer
    ...
    if (ret == 0) { /* success */
        printf("original len = %ld, len_inf = %lu\n",
               lSize, len_inf);
    } else { /* failure */
